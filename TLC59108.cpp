    ...
    TLC59108::TLC59108(PinName p_sda, PinName p_scl, const uint8_t selectable_address):
    ...
    uint8_t TLC59108::init(){
        ...
        uint8_t status = 0;
        status &= this->setRegister(TLC59108::REGISTER::MODE1::ADDR, TLC59108::REGISTER::MODE1::ALLCALL);
        status &= this->setLedOutputMode(TLC59108::LED_MODE::PWM_IND);//PWM_INDGRP);//
    ...
    uint8_t TLC59108::setBrightness(const uint8_t dutyCycle)
    {
        uint8_t status = 0;
        this->i2c->start();
        status &= this->i2c->write(addr);
        status &= this->i2c->write(TLC59108::REGISTER::PWM0::ADDR | TLC59108::AUTO_INCREMENT::IND);
        for (uint8_t i = 0; i < NUM_CHANNELS; i++)
            status &= this->i2c->write(dutyCycle);
        this->i2c->stop();
        return status;
    }
    
    uint8_t TLC59108::setBrightness(const uint8_t dutyCycles[], const uint8_t length) 
    {
        return setRegisters(TLC59108::REGISTER::PWM0::ADDR, dutyCycles, length);
    }
    ...
    int TLC59108::setRegisters(const uint8_t reg, const uint8_t values[], const uint8_t length) {
        uint8_t status = 0;
        this->i2c->start();
        status &= this->i2c->write(addr);
        status &= this->i2c->write(reg | TLC59108::AUTO_INCREMENT::ALL);
        for (uint8_t i = 0; i < length; i++)
            status &= this->i2c->write(values[i]);
        this->i2c->stop();
        return status;
    }